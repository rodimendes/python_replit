...
letra = input("Qual a letra desta rodada? ")[0].lower()
...
while jogo == True:
    ...
    for jogador in jogadores:
        palavra = input(f"{jogador}, é a sua vez de inserir uma palavra com a letra '{letra.upper()}': [Pressione '0' para sair.]\n").lower()
        if palavra == "0":
            jogo = False
        while palavra in lista_palavras:
            chance = 3
            palavra = input(f"{jogador}, esta palavra já foi escolhida. Pense em outra. Você só tem {chance} chances de inserir uma palavra com a letra '{letra.upper()}':\n").capitalize()
            chance -= 1
            if chance == 0:
                print('Você perdeu.\nFim do jogo!')
                jogo == False
        while palavra[0] != letra:
            palavra = input("A palavra não inicia com a letra escolhida. Tente outra:\n")
